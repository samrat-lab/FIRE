...
def fetch_data(exp_fname='expression_file_norm.csv',gene_fname='FDR_list_cutoff_old.05.xlsx',cat_col="labels"):
...
def get_models(rnd=xx):
    ...
    # random forest
    model = RandomForestClassifier(random_state=rnd,n_estimators=xx)
    models['rf'] = model
    ...
    # Logistic regression
    model = LogisticRegression(solver='lbfgs',max_iter=xxx,random_state=rnd)
    models['logit'] = model
    return models
...
with pd.ExcelWriter(output_file) as writer:
    ...
    for name, model in tqdm(models.items()):
        ...
        rkf=RepeatedStratifiedKFold(n_splits=xx, n_repeats=xx, random_state=rnd)
        cross_out = cross_validate(model, X, y,return_train_score=True,return_estimator=True ,scoring=('accuracy','f1','roc_auc','precision','recall'),n_jobs=xx ,cv=rkf)
