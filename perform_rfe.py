...
def get_models(st=xx,cvs=xx,rnd = xx,min_feat=x):
    ...
    #Decision Tree
    rfe = RFECV(estimator=DecisionTreeClassifier(random_state=rnd),step=st,min_features_to_select=min_feat,cv=cvs,n_jobs=xx)
    models['dt'] = rfe
    ...
    #Perceptron
    rfe = RFECV(estimator=Perceptron(),step=st,min_features_to_select=min_feat,cv=cvs,n_jobs=xx)
    models['per'] = rfe

    # svm
    rfe = RFECV(estimator=SVC(kernel='linear',random_state=rnd),step=st,min_features_to_select=min_feat,cv=cvs,n_jobs=xx)
    models['svm'] = rfe


    # XGBoost
    rfe = RFECV(estimator=XGBClassifier(random_state=rnd),step=st,min_features_to_select=min_feat,cv=cvs,n_jobs=xx)
    models['xgb'] = rfe


    #Some liner
    rfe = RFECV(estimator=LogisticRegression(solver='lbfgs',max_iter=1000,random_state=rnd),step=st,min_features_to_select=min_feat,cv=cvs,n_jobs=xx)
    models['logit'] = rfe
